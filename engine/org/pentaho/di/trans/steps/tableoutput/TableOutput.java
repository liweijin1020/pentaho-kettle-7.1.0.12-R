...
public class TableOutput extends BaseStep implements StepInterface {
  ...
  public boolean processRow(StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    ...
    // 获取到上一个步骤的输入行
    Object[] r = getRow(); // this also waits for a previous step to be finished. 这也要等待上一步完成。
    if ( r == null ) { // no more input to be expected... 没有值
      // truncate the table if there are no rows at all coming into this step
      if ( first && meta.truncateTable() ) {// 如果没有行进入此步骤，则截断表
        truncateTable();
      }
      return false;
    }
