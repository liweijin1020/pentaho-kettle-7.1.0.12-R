...
public class TableInput extends BaseStep implements StepInterface {
  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    if ( first ) { // we just got started
      ...
      // Make sure we read data from source steps... 确保我们从源步骤读取数据...
      if ( data.infoStream.getStepMeta() != null ) {
        if ( meta.isExecuteEachInputRow() ) {
          ...
          RowMetaAndData rmad = readStartDate(); // Read values in lookup table (look)  读取查找表中的值（查找）
      ...
      if ( meta.isExecuteEachInputRow() && ( parameters == null || parametersMeta.size() == 0 ) ) {
        setOutputDone(); // signal end to receiver(s) 信号端到接收器
        return false; // stop immediately, nothing to do here. 立即停止，这里无事可做。
      }
      ...
      if ( data.thisrow != null ) { // We can expect more rows
        ...
        try {
          ...
          if ( e.getCause() instanceof SQLException && isStopped() ) {
            //This exception indicates we tried reading a row after the statment for this step was cancelled  此异常表示在取消此步骤的语句后，我们尝试读取一行
            //this is expected and ok so do not pass the exception up  不要将异常传递出去
    ...
    if ( data.thisrow == null ) { // Finished reading?
      ...
      boolean done = false;
      if ( meta.isExecuteEachInputRow() ) { // Try to get another row from the input stream  尝试从输入流中获取另一行
        Object[] nextRow = getRowFrom( data.rowSet );
        if ( nextRow == null ) { // Nothing more to get!

          done = true;
        } else {
          // First close the previous query, otherwise we run out of cursors!  首先关闭上一个查询，否则游标将用完！
          closePreviousQuery();
