...
public class RunThread implements Runnable {
  ...
  public void run() {
    try {
      ...
      step.dispose( meta, data );// Finished reading query, closing connection.
      ...
      try {
        ...
        if ( li > 0 || lo > 0 || lr > 0 || lw > 0 || lu > 0 || lj > 0 || e > 0 ) {
          log.logBasic( BaseMessages.getString( PKG, "BaseStep.Log.SummaryInfo", String.valueOf( li ),
            ...
            String.valueOf( lu ), String.valueOf( e + lj ) ) ); // 表输出.0 - 完成处理 (I=0, O=2, R=2, W=2, U=0, E=0)   表输入.0 - 完成处理 (I=2, O=0, R=0, W=2, U=0, E=0)
        } else {
        ...
        step.markStop();// Shutting down heartbeat signal for tableinput
      }
    }
  }
}
