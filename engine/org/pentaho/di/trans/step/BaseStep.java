...
public class BaseStep implements VariableSpace, StepInterface, LoggingObjectInterface, ExtensionDataInterface {
   ...
   * each rowset!
   *
   * putrow用于复制一行，对于备用行集，这应该优先于其他所有行集！（synchronized）如果Distribute为true，则一行只复制一次到输出行集，否则将向每个行集发送副本！
   * @param row
  ...
  private void noPartitioning( RowMetaInterface rowMeta, Object[] row ) throws KettleStepException {
    if ( distributed ) {
      if ( rowDistribution != null ) {
        ...
        incrementLinesWritten();// 将写入下一步的行数增加一

        // Now determine the next output rowset! 现在确定下一个输出行集！
        // Only if we have more then one output... 只有我们有一个以上的输出...
        //
   ...
   * 对于getRow，我们通过输入行集接收来自先前步骤的数据。如果我们拆分流，则必须将数据复制到备用拆分：行集1到n。
   */
  @Override
  public Object[] getRow() throws KettleException {
    ...
    // 是否需要切换到下一个输入流?
    if ( blockPointer >= NR_OF_ROWS_IN_BLOCK ) {// NR_OF_ROWS_IN_BLOCK = 500

      // Take a peek at the next input stream.If there is no data, process another NR_OF_ROWS_IN_BLOCK on the next input stream.
      // 看看下一个输入流，如果没有数据，请在下一个输入流上处理另一个NR_OF_ROWS_IN_BLOCK。
      ...
      // What's the current input stream? 当前输入流是什么？
      inputRowSet = currentInputStream();
    }

    // To reduce stress on the locking system we are going to allow The buffer to grow beyond "a few" entries. We'll only do that if the previous step has not ended...
    //为了减少对锁定系统的压力，我们将允许缓冲区增加到“几个”条目之外。只有在上一步尚未结束时，我们才会这样做...
    ...
    // See if this step is receiving partitioned data... 查看此步骤是否正在接收分区数据...
    // In that case it might be the case that one input row set is receiving all data and the other rowsets nothing. (repartitioning on the same key would do that)
    // 在这种情况下，可能是一个输入行集正在接收所有数据，而其他行集却什么也不接收。 （对相同的键进行分区可以做到这一点）

    // We never guaranteed that the input rows would be read one by one alternatively. 我们从不保证输入行将被一一读取。
    // So in THIS particular case it is safe to just read 100 rows from one rowset, then switch to another etc. 因此，在这种特殊情况下，仅从一个行集中读取100行，然后切换到另一行是安全的。
    // We can use timeouts to switch from one to another... 我们可以使用超时从一个切换到另一个...
    //
    while ( row == null && !isStopped() ) {
      // Get a row from the input in row set ... 从行集中的输入获取一行...
      // Timeout immediately if nothing is there to read. 如果没有要读取的内容，则立即超时。
      // We will then switch to the next row set to read from... 然后，我们将切换到下一行以读取...
      //
      ...
      if ( row != null ) {
        ...
        // Try once more... 再试一次...
        // If row is still empty and the row set is done, we remove the row set from the input stream and move on to the next one...
        // 如果row仍然为空并且行集已完成，则从输入流中删除该行集，然后继续进行下一个...
        if ( inputRowSet.isDone() ) {
          row = inputRowSet.getRowWait( 1, TimeUnit.MILLISECONDS );
          if ( row == null ) {
            inputRowSets.remove( currentInputRowSetNr );
            if ( inputRowSets.isEmpty() ) {
              return null; // We're completely done. 已经全部完成了。
            }
          } else {
            incrementLinesRead();
          }
    ...
    while ( row == null && !stopped.get() ) {
      // Try the next input row set(s) until we find a row set that still has rows... 尝试下一个输入行集，直到找到仍然有行的行集。
      // The getRowFrom() method removes row sets from the input row sets list.  这个getRowFrom（）方法从输入行集列表中删除行集。
      //
      if ( inputRowSets.isEmpty() ) {
    ...
    // Also set the meta data on the first occurrence. 还要在第一次出现时设置元数据。
    // or if prevSteps.length > 1 inputRowMeta can be changed 或者如果prevSteps.length> 1 inputRowMeta可以更改
    ...
    // Check the rejection rates etc. as well. 还要检查废品率等。
    verifyRejectionRates();
